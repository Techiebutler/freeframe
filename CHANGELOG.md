...
- **A retried `/upload/complete` must present the upload id it was given at initiate.** A request carrying an unrecognised upload id is now refused as a replay rather than answered from storage. The browser client always sends back the id it was handed, so it is unaffected; versions created before upload ids were recorded are still accepted.
...
- **Retrying a completion whose response was lost no longer reports a successful upload as failed** — `/upload/complete` answered any already-finished version with 409, including the common case where the first call committed `processing` and the response never made it back. The idempotency check that exists for exactly this (`_already_assembled`) sat behind the guard and was unreachable, so a retry told the user their upload had failed while the transcode of that same file was running; the client also fires `/upload/abort` from the catch of every completion failure. The endpoint now asks storage first: if the assembled object is at the key at the declared size, it reports the version's current status instead of a conflict. Replay protection is unchanged — the status write and the transcode dispatch still only ever run from `uploading` — and a version that is `failed`, or one whose object is short or absent, still gets the 409. A retry is told apart from a replay by the upload id the version recorded at initiate, so a replay is refused from stored state alone without touching storage. (#263 by @Lennart-Pingpong)
- **A storage blip during an upload no longer destroys the upload** — three calls on the completion and abort paths had no handling for never reaching the backend, as opposed to being refused by it, so a connection wobble surfaced as a 500. The client answers any completion failure by calling `/upload/abort`, which recorded the version as failed, and the stale-upload reaper then deleted a file that had transferred perfectly. Listing an upload's parts, completing it, and aborting it now all answer a `503` with `Retry-After` instead, and none of them writes a status on the way out. Aborting also no longer treats "storage did not answer" as "the object is not there": it leaves the version alone rather than recording a failure it cannot support. (#263 by @Lennart-Pingpong)
- **A transcode that could not be queued is recorded instead of vanishing** — the version was committed as `processing` before the task was handed to the broker, and a dispatch failure was only ever a log line, so a broker outage left an upload stuck in `processing` forever: nothing sweeps that state, there is no reprocess endpoint, and the bytes kept counting against the storage cap. The version is now marked failed when its transcode cannot be queued, so it shows up and can be re-uploaded. (#263 by @Lennart-Pingpong)
- **`failed` now means the transcode is not coming back** — it was written before each retry, so a version oscillated between `failed` and `processing` for the whole retry ladder while its file sat there intact, and anything keying off the status got a different answer depending on when it asked. It is written once the retries are spent. (#263 by @Lennart-Pingpong)
- **Two completions of the same upload can no longer queue two transcodes** — the check that only an `uploading` version may be completed was an unsynchronised read, so concurrent requests could both pass it and both dispatch, with two transcodes writing the same processed prefix at once. The version is now claimed with a conditional update and only the request that claimed it dispatches. (#263 by @Lennart-Pingpong)

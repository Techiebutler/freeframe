...
### Added
- **Short share URLs** — every share link is now issued a 4-character base62 code alongside its token, so links look like `https://your-domain.example/6RBr` instead of `https://your-domain.example/share/<46-char-token>`. Codes resolve in the web app itself: a root-level route looks the code up through `GET /resolve/{short_code}` and redirects to the share page, so no reverse-proxy configuration is needed on any deployment (sub-path installs get `https://host/freeframe/6RBr`). Codes never take the name of a web root route. Short URLs are shown in the dashboard, the link-created screen, and share emails whenever present, with the full token URL as the fallback. Existing links have no code until `scripts/backfill_short_codes.py` is run once. (#178)

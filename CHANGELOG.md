...
### Fixed
- **An upload whose completion response was lost is no longer reported as Failed** — the client awaited `/upload/complete` once and treated any error as the upload having failed, but a completion whose response went missing on the way back (a dropped connection, a closed laptop, a proxy timeout) has already been committed and its transcode started. The upload showed as Failed while the file was busy processing, and the client sent `/upload/abort` behind it asking for that version to be discarded. It now asks the server what actually became of the version before deciding, and leaves it alone when the answer is that the completion landed. The check is keyed on the version, so an interrupted new version of an existing asset is still correctly reported as failed rather than reading its predecessor's status as success. (#269)
